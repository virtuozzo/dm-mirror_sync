...
static void read_callback(unsigned long error, void *context)
{
	...
	if (unlikely(error)) { /* READ ERROR HANDLING! */
		...
		if ((error == -EOPNOTSUPP) ||
			((error == -EWOULDBLOCK) && bio_rw_flagged(bio, BIO_RW_AHEAD)) ) {
							...
							unsigned long bsize, struct page **pages, int rw)
{
	...
	bio->bi_rw = READ | (1 << BIO_RW_SYNCIO);
	bio->bi_vcnt = 0;
	bio->bi_idx = 0;
			 ...
			 char *result, unsigned int maxlen)
{
	...
	case STATUSTYPE_INFO:
		...
		DMEMIT("DMS C6 [Build: %s %s] ", __DATE__, __TIME__);
...
static int mirror_sync_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	ti->private = ms;
 	ti->split_io = 1 << 13; /* sectors == 4 MB... used to be dm_rh_get_region_size(ms->rh); */
...
static int __init dm_mirror_sync_init(void)
{
	...
	_dms_mirror_sync_record_cache = KMEM_CACHE(dms_bio_map_info, 0);
	...
	printk(KERN_INFO "DMS C6 [Build: %s %s]: Loaded OK.\n", __DATE__, __TIME__);
...
{
	printk(KERN_INFO "DMS C6 [Build: %s %s]: Exiting.\n", __DATE__, __TIME__);
