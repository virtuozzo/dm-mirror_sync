...
if [ ! -b $dms_device ] ; then
	...
	echo 'DMS Created OK!'
	echo DMS Size: `/sbin/blockdev --getsize $dms_device`
	echo "Creating DM Linear device & map..."
	echo "/sbin/dmsetup create $ldms_devname --table '0 $mindevsize linear /dev/mapper/$dms_devname 0'"
	/sbin/dmsetup create $ldms_devname --table "0 $mindevsize linear /dev/mapper/$dms_devname 0"
	echo 'DMS Created OK!'
