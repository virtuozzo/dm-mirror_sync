...
/* NOTE: we MUST return an allocated structure, or panic will follow... */
static struct dms_bio_map_info *
dms_bio_map_info_pool_alloc( struct mirror_sync_set *ms )
{
	struct dms_bio_map_info *bmi = NULL;
	unsigned retries = 0;

	bmi = kmalloc( sizeof(struct dms_bio_map_info), GFP_NOIO );

	while ( unlikely(!bmi) ) {

		if ( !ms->errmsg_last_time || jiffies >= ms->errmsg_last_time + (6*HZ) ) {
			DMERR("[%s] Failed to allocate memory, retrying (%d)", ms->name, retries++ );
			ms->errmsg_last_time = jiffies;
		}

		/* wait and retry until we get some memory... */
		msleep( 100 ); /* no busy waiting as in mdelay() ! */
		bmi = kmalloc( sizeof(struct dms_bio_map_info), GFP_NOIO );
	}
...
static void
dms_bio_map_info_pool_free( struct dms_bio_map_info *bmi )
{
	assert(bmi);
	kfree(bmi);
}
...
{
	struct mirror_sync_set *ms = data;
...
/* Contrary to dm_table_unplug_all() this unplug only one device! */

#ifndef DISABLE_UNPLUGS
void dms_unplug_dev(struct mirror *m)
{
	request_queue_t *q = bdev_get_queue(m->dev->bdev);
	...
	if (likely(q)) {
		...
		DMWARN("[%s] Cannot unplug nonexistent device %s",
				m->ms->name, bdevname(m->dev->bdev, b));
	}
}
#endif
 ...
 * here, the caller should have set the proper map_context data for retries & faults ... */
...
static int write_async_bios( struct dms_bio_map_info *bmi, struct bio *bio)
{
	...
	/*DMSDEBUGX("DMS REQ [2]: WR Addr: %lld Size: %d - LIVE: %d - %d-%d-%d %s-%s-%s\n",
				(unsigned long long)bio->bi_sector << 9, bio->bi_size, nr_live,
...
#else
	(void) dm_io(&io_req, nr_live, io, NULL);
#endif
...
static void read_callback(unsigned long error, void *context)
{
	...
	if (unlikely(error)) { /* READ ERROR HANDLING! */
		...
		if ((error == -EOPNOTSUPP) ||
			((error == -EWOULDBLOCK) && bio_rw_ahead(bio)) ) {
		...
		DMWARN("[%s] Mirror device %s: Read I/O failure [Addr: %lld Size: %d] ...handling it",
				m->ms->name, m->dev->name, (unsigned long long)bio->bi_sector << 9, bio->bi_size);
		...
		if ( likely(mirror_sync_available(m->ms)) ) {
			...
			DMWARN("[%s] Read failure [Addr: %lld Size: %d] - Trying different mirror",
					m->ms->name, (unsigned long long)bio->bi_sector << 9, bio->bi_size);
		      ...
		      union map_info *map_context)
{
	...
	bmi = dms_bio_map_info_pool_alloc( ms );
	...
	if (rw == WRITE) {
		...
		/*
		 * CAUTION: DO NOT SET map_context->ll, it overwrites map_context->ptr!!
		 */
...
#ifndef DISABLE_UNPLUGS
		dm_table_unplug_all(ms->ti->table); /* ESSENTIAL for speed... */
#endif
		return 0;
	}
	...
	m = choose_read_mirror(ms, bio->bi_sector);
...
#ifndef DISABLE_UNPLUGS
		dms_unplug_dev(m); /* ESSENTIAL for speed... */
#endif
		return 0;
...
write_all_dead:
		...
		/* must free struct here, no callback is called... */
		dms_bio_map_info_pool_free( bmi );
...
static int mirror_sync_end_io(struct dm_target *ti, struct bio *bio,
			 int error, union map_info *map_context)
{
	...
	// -> IF assertion fails -> CRASH COMING UP!
	if (unlikely(!map_context->ptr)) {
		assert(map_context->ptr); /* CANNOT handle sync mirroring without an I/O record?! */
		DMERR("[%s] BUG: Unable to complete write call - NULL pointer.", ms->name);
		return -EIO;
	}
	...
	/* IMPORTANT: we must free the map_contect allocated in case of failure... */
	dms_bio_map_info_pool_free( map_context->ptr );
...
static void do_read_failures(struct mirror_sync_set *ms, struct bio_list *read_failures)
{
	...
	while ((bio = bio_list_pop(read_failures))) {
		...
		if (likely(bmi)) {
			...
			bio_endio(bio, bio->bi_size, -EFAULT);
			continue;
		}
 ...
 *---------------------------------------------------------------*/
static void main_mirror_syncd(void *data)
{
	struct mirror_sync_set *ms = (struct mirror_sync_set *) data;
...
static int mirror_sync_message(struct dm_target *ti, unsigned argc, char **argv)
{
	...
	if ( strncmp(argv[0], "io_cmd", strlen(argv[0])) == 0 ) {
		...
		if ( strncmp(argv[1], "set_weight", strlen(argv[1])) == 0 ) {
			...
			if (sscanf(argv[2], "%u", &devno) != 1 || devno < 0 ||
						devno >= ms->nr_mirrors) {
				DMERR("[%s] Invalid device number (arg 3): has to between 0 - %d",
						ms->name, ms->nr_mirrors );
				return -EINVAL;
			}
			if (sscanf(argv[3], "%u", &value) != 1 || value < 1 || value > 100 ) {
		 ...
		 */
			...
			if (sscanf(argv[3], "%u", &value) != 1 || value < 4096 ||
			...
			if (sscanf(argv[2], "%llu", &llvalue) != 1 || llvalue < 0 || llvalue >= ti->len ) {
		...
		if ( strncmp(argv[1], "round_robin", strlen(argv[1])) == 0 ) {
			...
			if (sscanf(argv[3], "%u", &value) != 1 || value < 2 ||
			...
			if (sscanf(argv[3], "%u", &value) != 1 || value < 128 ||
						value % 8 ) {
...
static int mirror_sync_status(struct dm_target *ti, status_type_t type,
			 char *result, unsigned int maxlen)
{
	unsigned int m, sz = 0;
	struct mirror_sync_set *ms = (struct mirror_sync_set *) ti->private;
	...
	case STATUSTYPE_INFO:
		...
		DMEMIT("DMS C5 [Build: %s %s] ", __DATE__, __TIME__);
		      ...
		      unsigned int mirror, char **argv)
{
	...
	DMSDEBUG("getmirror: %s off:%lld len:%lld mode:%d\n", argv[0], offset, ti->len,
					  ...
					  read_policy_params_t *rp )
{
	...
	*args_used = 2 + param_count;
	...
	if ( strlen(argv[0]) == 4 && strncmp(argv[0], "core", 4) == 0 ) {
		...
		if (sscanf(argv[2], "%u", &value) != 1 || value < 2 || value > 1024*1024*1024) {
		...
		*args_used = 2 + param_count;
		...
		if (sscanf(argv[2], "%u", &value) != 1 || value < 128 || value % 8 ) {
		...
		*args_used = 2 + param_count;
		...
		if ( (sscanf(argv[2], "%u", &allweights) != 1 || allweights < 1 || allweights > 100) ||
			 (sscanf(argv[4], "%u", &weightx) != 1 || weightx < 1 || weightx > 100) ) {

			ti->error = "Invalid device weights: must be between 1 - 100";
			return 0;
		}
		if (sscanf(argv[5], "%u", &value) != 1 || value > 16 ||
			sscanf(argv[3], "%u", &devx) != 1 || devx >= value ) {
...
preserve_ms_params_on_reconfig( int new_ms_idx, char *devname )
{
	...
	if ( oidx >= 0 ) { /* found something... */
		...
		if ( ods && nds )
			DMINFO("[%s] RECONFIG: %d %s-> NEW %d %s",
				oldms->name, oldms->nr_mirrors, ods, newms->nr_mirrors, nds );
...
static int mirror_sync_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	ti->private = ms;
 	ti->split_io = 1 << 13; /* sectors == 4 MB... used to be ms->rh.region_size; */
	...
	INIT_WORK(&ms->kmirror_syncd_work, main_mirror_syncd, ms);
	//init_timer(&ms->timer);
	ms->timer_pending = 0;
...
static int __init dm_mirror_sync_init(void)
{
	...
	_dms_mirror_sync_record_cache = kmem_cache_create("dms_bio_map_info",
						sizeof(struct dms_bio_map_info), 0, 0, NULL, NULL);
	if (!_dms_mirror_sync_record_cache) {
		r = -ENOMEM;
		goto bad_cache;
	}
	...
	printk(KERN_INFO "DMS C5 [Build: %s %s]: Loaded OK.\n", __DATE__, __TIME__);
...
bad_target2:
	kfree( reconf_ms );
bad_target:
	kmem_cache_destroy(_dms_mirror_sync_record_cache);
bad_cache:
	return r;
}
...
static void __exit dm_mirror_sync_exit(void)
{
	...
	printk(KERN_INFO "DMS C5 [Build: %s %s]: Exiting.\n", __DATE__, __TIME__);

	r = dm_unregister_target(&mirror_sync_target);
	if (r < 0)
		DMERR("[%s] unregister failed %d", mirror_sync_target.name, r);
